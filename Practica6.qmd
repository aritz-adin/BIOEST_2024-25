...
cangrejos <- data.frame(anchura, satelites, total)
print(cangrejos)
```
...
```{r include=FALSE}
modelo.lm <- lm(prop~anchura, weights=total, data=cangrejos)
summary(modelo.lm)
```

-   **¿Cuáles son las estimaciones de los coeficientes? ¿Son estadísticamente significativos?**

    <!-- Las estimaciones de los coeficientes del modelo son: $\hat{\alpha}=`r round(as.numeric(modelo.lm$coefficients[1]),3)`$ y $\hat{\beta}=`r round(as.numeric(modelo.lm$coefficients[2]),3)`$ -->
...
-   **¿Cuál es el coeficiente de determinación? Explica su significado.**
    ...
    <!-- $R^2_\mbox{adjusted}=`r round(summary(modelo.lm)$adj.r.squared,4)`$. Es decir, el `r 100*round(summary(modelo.lm)$adj.r.squared,4)`% de la variabilidad total es explicada por el modelo de regresión (variable`anchura). -->
...
data.frame(anchura=cangrejos$anchura,
           prop=cangrejos$prop,
           prop.pred=fitted(modelo.lm),
           satelite=cangrejos$satelites,
           satelite.pred=fitted(modelo.lm)*cangrejos$total)
```
...
abline(modelo.lm, col="red", lwd=2)
```
...
```{r include=FALSE}
modelo.glm <- glm(prop~anchura, weights=total, family=binomial, data=cangrejos)
summary(modelo.glm)
```

-   **¿Cuáles son las estimaciones de los coeficientes? ¿Son estadísticamente significativos?**

    <!-- Las estimaciones de los coeficientes del modelo son: $\hat{\alpha}=`r round(as.numeric(modelo.glm$coefficients[1]),3)`$ y $\hat{\beta}=`r round(as.numeric(modelo.glm$coefficients[2]),3)`$ -->
...
```{r, include=FALSE}
beta <- coef(modelo.glm)[2]
hat.prop <- predict(modelo.glm, newdata=data.frame(anchura=26), type="response")
...
tasa.incremental(modelo.glm, 26, 27, decimales=4)
```
...
```{r, include=FALSE}
tasa.incremental(modelo.glm, 23, 24, decimales=4)
tasa.incremental(modelo.glm, 25, 26, decimales=4)
```
...
```{r, include=FALSE}
exp(coef(modelo.glm)[2])
```

<!-- Es decir, por cada incremento de 1 cm en la anchura del caparazón el odds estimado crece aproximadamente un `r round(exp(coef(modelo.glm)[2])-1,2)*100`%. -->
...
IC.beta <- confint(modelo.glm)[2,]
IC.beta

IC.OR <- c(exp(IC.beta[1]), exp(IC.beta[2]))
IC.OR
```
...
data.frame(anchura=cangrejos$anchura,
           prop=cangrejos$prop,
           prop.pred=fitted(modelo.glm),
           satelite=cangrejos$satelites,
           satelite.pred=fitted(modelo.glm)*cangrejos$total)
```
...
```{r, include=FALSE}
data.pred <- data.frame(anchura=seq(23,30,0.01))
data.pred$prop <- predict(modelo.glm, data.pred, type="response")

plot(cangrejos$anchura, cangrejos$prop, xlab="Anchura (cm)", ylab="Prop", ylim=c(-0.1,1.1),
     main="Modelo regresión logística")
abline(h=c(0,1), lty=2)
abline(modelo.lm, col="red", lwd=2)
lines(data.pred$anchura, data.pred$prop, col="blue", lwd=2)
...
<!-- ### 2.11. Calcula la probilidad estimada y su intervalo de confianza al 95\% de que un insecto fallezca con una dosis de 185 unidades. -->
